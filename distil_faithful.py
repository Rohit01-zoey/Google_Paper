...
def data(subset_size,data_set):
    ...
    return train_dataloader_l, train_dataloader_u, val_dataset,test_dataloader,num_cls

def set_seed(seed=753410):
    np.random.seed(seed)
    random.seed(seed)
...
def main(args):
    ...
    CUDA = "cuda:"+args.cuda
    set_seed(args.seed)
    train_dataloader_l, train_dataloader_u, val_dataset, test_dataloader,num_cls = data(float(args.subset_size),args.data_set)
    ...
    if args.data_set in ['cifar10', 'cifar100', 'svhn', 'celeb_a']:
        ...
        optimizer =torch.optim.SGD(student.parameters() , lr=0.1,momentum=0.2, nesterov=False)
